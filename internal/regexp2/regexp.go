...
const (
	...
	Unicode                              = 0x0400 // "u"
	JavaASCII                            = 0x0800 // "a": Java CASE_INSENSITIVE
	JavaUnicode                          = 0x1000 // "j": Java CASE_INSENSITIVE with UNICODE_CASE
)
...
// FindRunesMatchContext searches the input rune slice and interrupts
// backtracking when ctx is canceled.
func (re *Regexp) FindRunesMatchContext(ctx context.Context, r []rune) (*Match, error) {
	return re.runContext(ctx, false, -1, r)
}
...
// FindRunesMatchStartingAtContext searches from startAt and interrupts
// backtracking when ctx is canceled.
func (re *Regexp) FindRunesMatchStartingAtContext(ctx context.Context, r []rune, startAt int) (*Match, error) {
...
func (re *Regexp) FindNextMatch(m *Match) (*Match, error) {
	return re.FindNextMatchContext(context.Background(), m)
}

// FindNextMatchContext continues a search and interrupts backtracking when ctx
// is canceled.
func (re *Regexp) FindNextMatchContext(ctx context.Context, m *Match) (*Match, error) {
	if m == nil {
		return nil, nil
	}
	...
	return re.runWithGraphemeStartContext(ctx, false, startAt, m.textpos, m.text)
}
