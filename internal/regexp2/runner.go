...
import (
	...
	"github.com/sliverarmory/opfor/internal/regexp2/syntax"
)
...
type runner struct {
	...
	// Lazily populated for contextual \b{g} assertions and reverse \X.
	// Forward \X computes from its actual start because OpenJDK treats that
	// position as the beginning of a cluster even when it is not a boundary in
	// the surrounding text.
	rungrapheme      []int
	rungraphemeFull  []int
	rungraphemestart int
...
func (re *Regexp) run(quick bool, textstart int, input []rune) (*Match, error) {
	return re.runContext(context.Background(), quick, textstart, input)
}
...
func (re *Regexp) runWithGraphemeStart(quick bool, textstart, graphemeStart int, input []rune) (*Match, error) {
	return re.runWithGraphemeStartContext(context.Background(), quick, textstart, graphemeStart, input)
}
...
func (r *runner) scan(ctx context.Context, rt []rune, textstart, reverseFloor, graphemeStart int, quick bool, timeout time.Duration) (*Match, error) {
	...
	r.rungraphemestart = graphemeStart
...
func (r *runner) execute() error {
	...
	for {
		...
		case syntax.GraphemeBoundary:
			boundary, err := r.isJavaGraphemeBoundary(r.textPos())
			if err != nil {
				return err
			}
			if !boundary {
		...
		case syntax.Grapheme:
			if !r.rightToLeft {
				...
				next, err := nextJavaGraphemeBoundary(r.runtext, r.textPos(), r.runtextend, r.checkTimeout)
				if err != nil {
					return err
				}
				r.textto(next)
			} else {
				previous, err := r.previousJavaGraphemeBoundary(r.textPos())
				if err != nil {
					return err
				}
				if previous == r.textPos() {
					break
				}
				r.textto(previous)
			}
		...
		case syntax.Set:
			...
			if r.forwardchars() < 1 || !r.charInSet(r.code.Sets[r.operand(0)], r.forwardcharnext()) {
				break
			}
		...
		case syntax.Setrep:
			...
			for c > 0 {
				if !r.charInSet(set, r.forwardcharnext()) {
					goto BreakBackward
				}
				c--
			}
		...
		case syntax.Setlazy | syntax.Back:
			...
			if !r.charInSet(r.code.Sets[r.operand(0)], r.forwardcharnext()) {
				break
			}
...
func (r *runner) setOperator(op int) {
	switch {
	case op&syntax.CiASCII != 0:
		r.caseMode = syntax.CaseJavaASCII
	case op&syntax.CiJava != 0:
		r.caseMode = syntax.CaseJavaUnicode
	case op&syntax.Ci != 0:
		r.caseMode = syntax.CaseInsensitive
	default:
		r.caseMode = syntax.CaseSensitive
	}
	r.rightToLeft = (0 != (op & syntax.Rtl))
	r.operator = syntax.InstOp(op & ^(syntax.Rtl | syntax.CaseMask))
}
...
func (r *runner) forwardcharnext() rune {
	...
	return syntax.FoldCase(ch, r.caseMode)
}

func (r *runner) charInSet(set *syntax.CharSet, ch rune) bool {
	if r.caseMode == syntax.CaseJavaASCII || r.caseMode == syntax.CaseJavaUnicode {
		return set.CharInCase(ch, r.caseMode)
	}
	return set.CharIn(ch)
}
...
func (r *runner) runematch(str []rune) bool {
	...
	for c != 0 {
		c--
		pos--
		if str[c] != syntax.FoldCase(r.runtext[pos], r.caseMode) {
			return false
		}
	}
...
func (r *runner) refmatch(index, len int) bool {
	...
	for c != 0 {
		...
		if syntax.FoldCase(r.runtext[cmpos], r.caseMode) != syntax.FoldCase(r.runtext[pos], r.caseMode) {
			return false
		}
	}
...
func (r *runner) findFirstChar() bool {
	...
	if r.code.FcPrefix.CaseInsensitive {
		r.caseMode = syntax.CaseInsensitive
	} else {
		r.caseMode = syntax.CaseSensitive
	}
...
func (r *runner) isJavaGraphemeBoundary(index int) (bool, error) {
	if index < 0 || index > r.runtextend {
		return false, nil
	}
	if index == 0 || index == r.runtextend {
		return true, nil
	}
	if index <= r.rungraphemestart {
		return false, nil
	}
	if err := r.ensureJavaGraphemeBoundariesThrough(&r.rungrapheme, r.rungraphemestart, index); err != nil {
		return false, err
	}
	position := sort.SearchInts(r.rungrapheme, index)
	return position > 0 && position < len(r.rungrapheme) && r.rungrapheme[position] == index, nil
}

func (r *runner) previousJavaGraphemeBoundary(index int) (int, error) {
	if index <= 0 {
		return index, nil
	}
	if err := r.ensureJavaGraphemeBoundariesThrough(&r.rungraphemeFull, 0, index); err != nil {
		return 0, err
	}
	position := sort.SearchInts(r.rungraphemeFull, index)
	if position >= len(r.rungraphemeFull) || r.rungraphemeFull[position] != index {
		return index, nil
	}
	if position == 0 {
		return index, nil
	}
	return r.rungraphemeFull[position-1], nil
}

func (r *runner) ensureJavaGraphemeBoundariesThrough(boundaries *[]int, start, target int) error {
	if len(*boundaries) == 0 {
		*boundaries = append(*boundaries, start)
	}
	offset := (*boundaries)[len(*boundaries)-1]
	for clusters := 0; offset < r.runtextend && offset < target; clusters++ {
		if clusters&1023 == 0 {
			if err := r.checkTimeout(); err != nil {
				return err
			}
		}
		next, err := nextJavaGraphemeBoundary(r.runtext, offset, r.runtextend, r.checkTimeout)
		if err != nil {
			return err
		}
		if next <= offset {
			return fmt.Errorf("regexp2: grapheme engine did not advance at %d", offset)
		}
