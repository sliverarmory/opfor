...
func TestSleepRegexLoneSurrogateOfficialJARDifferential(t *testing.T) {
	...
	if jar == "" {
		t.Skip("set OPFOR_SLEEP_JAR to the official Sleep 2.1 JAR for UTF-16 regex differential verification")
	}
	...
	javaOutput, err := osexec.Command(java, "-jar", jar, "-e", utf16RegexProbeSource).CombinedOutput()
	if err != nil {
