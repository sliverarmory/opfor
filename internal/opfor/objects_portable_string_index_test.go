...
func TestPortableJavaStringCodePointIndexOfficialJARDifferential(t *testing.T) {
	...
	if jar == "" {
		t.Skip("set OPFOR_SLEEP_JAR to the official Sleep 2.1 JAR for code-point index differential verification")
	}
	...
	reference, err := osexec.Command(java, "-Dfile.encoding=UTF-8", "-jar", jar, "-e", portableJavaStringCodePointIndexProbeSource).CombinedOutput()
	if err != nil {
