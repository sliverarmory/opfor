...
func (instance *portableScriptInstance) newChildRuntime(warnings io.Writer) (*Runtime, error) {
	...
	options := []Option{
		...
		WithBeaconStringEncoder(parent.beaconEncoder),
		WithAggressorEventDispatcher(parent.eventDispatcher),
		WithTaintMode(parent.taintMode),
	}
	if !isNilInterface(parent.variableProvider) {
		options = append(options, WithVariableProvider(parent.variableProvider))
	}
	// An explicitly configured transcript boundary follows portable child
	// runtimes. A nil parent sink remains nil so the child presents records
	// through its own console router instead of the parent's stdout.
	if !isNilInterface(parent.aggressorBeaconTranscriptSink) {
		options = append(options, WithAggressorBeaconTranscriptSink(parent.aggressorBeaconTranscriptSink))
	}
	// An explicit Beacon action provider follows portable children. Callback
	// capabilities remain owned by the child Runtime and creating Script even
	// when the importer provider instance is shared with its parent.
	if !isNilInterface(parent.aggressorBeaconActionProvider) {
		options = append(options, WithAggressorBeaconActionProvider(parent.aggressorBeaconActionProvider))
	}
	// An explicit low-level Beacon execution provider follows portable children.
	// Result callbacks remain owned by the child Runtime and creating Script
	// even when the importer provider instance is shared with its parent.
	if !isNilInterface(parent.aggressorBeaconExecutionProvider) {
		options = append(options, WithAggressorBeaconExecutionProvider(parent.aggressorBeaconExecutionProvider))
	}
	// An explicit BOF extractor follows portable children. Requests retain child
	// Runtime and Script provenance even though the importer extractor instance
	// is shared with its parent.
	if !isNilInterface(parent.aggressorBOFExtractor) {
		options = append(options, WithAggressorBOFExtractor(parent.aggressorBOFExtractor))
	}
	// An explicit artifact provider follows portable children. A stageless
	// callback remains owned by the child Runtime and creating Script even when
	// the importer provider instance is shared with its parent.
	if !isNilInterface(parent.aggressorArtifactProvider) {
		options = append(options, WithAggressorArtifactProvider(parent.aggressorArtifactProvider))
	}
	// Explicit payload, listener, and Payload Store providers follow portable
	// children. Requests retain child Runtime and Script provenance while the
	// importer-owned provider instances are shared with the parent.
	if !isNilInterface(parent.aggressorPayloadProvider) {
		options = append(options, WithAggressorPayloadProvider(parent.aggressorPayloadProvider))
	}
	if !isNilInterface(parent.aggressorListenerProvider) {
		options = append(options, WithAggressorListenerProvider(parent.aggressorListenerProvider))
	}
	if !isNilInterface(parent.aggressorPayloadStoreProvider) {
		options = append(options, WithAggressorPayloadStoreProvider(parent.aggressorPayloadStoreProvider))
	}
	// An explicit site-delivery provider follows portable children. Requests
	// retain child Runtime and Script provenance even when the importer provider
	// instance is shared with the parent.
	if !isNilInterface(parent.aggressorSiteProvider) {
		options = append(options, WithAggressorSiteProvider(parent.aggressorSiteProvider))
	}
	// An explicit Team Server RPC provider follows portable children. Response
	// callbacks remain owned by the child Runtime and creating Script even when
	// the importer provider instance is shared with the parent.
	if !isNilInterface(parent.aggressorTeamServerRPCProvider) {
		options = append(options, WithAggressorTeamServerRPCProvider(parent.aggressorTeamServerRPCProvider))
	}
	// An explicit metadata provider follows portable children. A nil provider
	// remains nil so the child's native wrappers preserve the inherited Host
	// compatibility route.
	if !isNilInterface(parent.aggressorSessionQueryProvider) {
		options = append(options, WithAggressorSessionQueryProvider(parent.aggressorSessionQueryProvider))
	}
	// An explicit Cobalt data-model provider follows portable children. With no
	// provider, the child keeps the inherited Host compatibility route.
	if !isNilInterface(parent.aggressorDataModelQueryProvider) {
		options = append(options, WithAggressorDataModelQueryProvider(parent.aggressorDataModelQueryProvider))
	}
	// An explicit application data-store provider follows portable children.
	// Requests retain child Runtime and Script provenance while Values keep
	// their original compound and object identity.
	if !isNilInterface(parent.aggressorDataStoreProvider) {
		options = append(options, WithAggressorDataStoreProvider(parent.aggressorDataStoreProvider))
	}
	// An explicit preference provider follows portable children. Requests retain
	// child Runtime and Script provenance while resolved Values keep their
	// ordinary scalar or array identity.
	if !isNilInterface(parent.aggressorPreferenceProvider) {
		options = append(options, WithAggressorPreferenceProvider(parent.aggressorPreferenceProvider))
	}
	// An explicit code-transform provider follows portable children. Requests
	// retain child Runtime and Script provenance while the importer owns the
	// exact encoded, compressed, or VBS-transformed output.
	if !isNilInterface(parent.aggressorCodeTransformProvider) {
		options = append(options, WithAggressorCodeTransformProvider(parent.aggressorCodeTransformProvider))
	}
	// An explicit process-injection configuration provider follows portable
	// children. Requests retain child Runtime and Script provenance even when
	// the importer provider instance is shared with the parent.
	if !isNilInterface(parent.aggressorProcessInjectionProvider) {
		options = append(options, WithAggressorProcessInjectionProvider(parent.aggressorProcessInjectionProvider))
	}
	// An explicit Team Server/profile provider follows portable children.
	// Requests retain child Runtime and Script provenance while profile-owned
	// transformations remain entirely importer-defined.
	if !isNilInterface(parent.aggressorProfileProvider) {
		options = append(options, WithAggressorProfileProvider(parent.aggressorProfileProvider))
	}
	// An explicit Team Server Covert VPN provider follows portable children.
	// Requests retain child Runtime and Script provenance even when the provider
	// instance is shared with the parent.
	if !isNilInterface(parent.aggressorVPNProvider) {
		options = append(options, WithAggressorVPNProvider(parent.aggressorVPNProvider))
	}
	// An explicit Cobalt-owned PE provider follows portable children. Requests
	// retain child Runtime and Script provenance while the provider owns all PE
	// parsing and transformation behavior.
	if !isNilInterface(parent.aggressorPEProvider) {
		options = append(options, WithAggressorPEProvider(parent.aggressorPEProvider))
	}
	// The connected-client service provider follows portable children. Requests
	// retain child Runtime and Script provenance even when the importer provider
	// instance is shared with the parent.
	if !isNilInterface(parent.aggressorClientServiceProvider) {
		options = append(options, WithAggressorClientServiceProvider(parent.aggressorClientServiceProvider))
	}
	// The client UI provider follows portable children. Any popup composer is
	// bound to the child Runtime's exact popup registration and component even
	// though the importer provider instance is shared with the parent.
	if !isNilInterface(parent.aggressorClientUIProvider) {
		options = append(options, WithAggressorClientUIProvider(parent.aggressorClientUIProvider))
	}
	// Explicit UI providers follow portable children. Their runtime-owned
	// dialog and prompt capabilities remain scoped to the child Runtime and
	// creating Script even when the importer provider instance is shared.
	if !isNilInterface(parent.aggressorDialogProvider) {
		options = append(options, WithAggressorDialogProvider(parent.aggressorDialogProvider))
	}
	if !isNilInterface(parent.aggressorPromptProvider) {
		options = append(options, WithAggressorPromptProvider(parent.aggressorPromptProvider))
	}
	// Breakpoint presentation follows portable children. The child interpreter
	// builds and detaches its own evaluator snapshot before the shared importer
	// provider is called, retaining child Runtime and Script provenance.
	if !isNilInterface(parent.aggressorBreakpointProvider) {
		options = append(options, WithAggressorBreakpointProvider(parent.aggressorBreakpointProvider))
	}
	if parent.aggressorCommands != nil {
		for _, kind := range []AggressorCommandKind{AggressorCommandBeacon, AggressorCommandSSH} {
			catalog := parent.aggressorCommands.baseSnapshot(kind)
			if len(catalog.Commands) != 0 || len(catalog.Groups) != 0 {
				options = append(options, WithAggressorCommandCatalog(kind, catalog))
			}
		}
	}
	if parent.aggressorBeaconTechniques != nil {
		for _, kind := range aggressorBeaconTechniqueKinds {
			catalog := parent.aggressorBeaconTechniques.baseSnapshot(kind)
			if len(catalog.Techniques) != 0 {
				options = append(options, WithAggressorBeaconTechniqueCatalog(kind, catalog))
			}
		}
	}
	instance.mu.Lock()
