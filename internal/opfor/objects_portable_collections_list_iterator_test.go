...
func TestPortableJavaListIteratorOfficialJARDifferential(t *testing.T) {
	...
	if jar == "" {
		t.Skip("set OPFOR_SLEEP_JAR to the official Sleep 2.1 JAR for ListIterator differential verification")
	}
	...
	reference, err := osexec.Command(java, "--add-opens=java.base/java.util=ALL-UNNAMED", "-Dfile.encoding=UTF-8", "-jar", jar, "-e", portableJavaListIteratorProbeSource).CombinedOutput()
	if err != nil {
...
func TestPortableJavaMapEntryDetachmentOfficialJARDifferential(t *testing.T) {
	jar := os.Getenv("OPFOR_SLEEP_JAR")
	if jar == "" {
		t.Skip("set OPFOR_SLEEP_JAR to the official Sleep 2.1 JAR for Map.Entry detachment differential verification")
	}
	jarBytes, err := os.ReadFile(jar)
	if err != nil {
		t.Fatal(err)
	...
	reference, err := osexec.Command(java, "--add-opens=java.base/java.util=ALL-UNNAMED", "-Dfile.encoding=UTF-8", "-jar", jar, "-e", portableJavaMapEntryDetachmentProbeSource).CombinedOutput()
	if err != nil {
